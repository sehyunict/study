...
- 메모리 사용 영역 `java`
	...
	<br> <br> <br>
** 참고
	> 스레드 : 한 프로그램의 작업 단위를 의미한다. <br>
	> 컴파일 타임 : 소스코드가 기계어로 변환되어 실행가능한 프로그램이 되는 과정(systenx Error, 파일 참조 오류, 타입 체크 오류 등) <br>
	> 런타임 : 컴파일 타임 이후 프로그램이 실행되는 때(0 나누기 오류, null 참조 오류, 메모리 부족 오류) <br>
	
** 멀티스레드 시 공유되는 메모리 영역은? <br>
	> 1,2번인 메소드 영역과 힙 영역은 모든 스레드가 공유한다. <br>
	> 3,4,5번은 각각의 스레드마다 생성되고 공유되지 않는다. <br>
